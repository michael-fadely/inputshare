...
public:
	...
	void pressKeyboardButton(VirtualButton button, bool down)
	{
		...
		{
			import x11.Xlib : XKeysymToString;
			auto keyString = XKeysymToString(toNative(button));

			if (down)
			{
				xdo_send_keysequence_window_down(handle, CURRENTWINDOW, keyString, 0);
			}
			else
			{
				xdo_send_keysequence_window_up(handle, CURRENTWINDOW, keyString, 0);
			}
		}
		else
		{
